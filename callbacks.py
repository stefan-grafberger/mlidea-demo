...
from mlidea.execution._patches import AppendNodeAfterOperator, DataProjection, OperatorReplacement, DataTransformer, \
...
# def run_pipeline(dag_extraction_result, *_shadow_pipelines, add_monkey_patching=False):
#     builder = PipelineAnalyzer.on_previously_extracted_pipeline(dag_extraction_result)
#
#     for analysis in _what_if_analyses:
#         builder = builder.add_what_if_analysis(analysis)
...
def scan_pipeline(pipeline_source_code, analysis_result, shadow_pipelines, database_path,
                  add_monkey_patching=False):
    if analysis_result is None:
        builder = PipelineAnalyzer.on_pipeline_from_string(pipeline_source_code)
    else:
        builder = PipelineAnalyzer.on_changed_pipeline_from_string(analysis_result.dag_extraction_info, pipeline_source_code)

    # builder = builder.set_intermediate_dag_storing(True)
    builder = builder.add_custom_monkey_patching_modules([custom_monkeypatching])
    builder = builder.add_shadow_pipelines(shadow_pipelines)
    builder = builder.set_function_transformer_cache_path(database_path)
...
def render_graph1(graph: nx.classes.digraph.DiGraph):
    ...
    G = get_original_simple_dag(graph, reuse_info)
    fig, _ = plt.subplots()
...
def render_graph3(graph: nx.classes.digraph.DiGraph):
    ...
    stylesheet = [
        ...
        #     "selector": "core",
        #     "style": {
        #         "background-color": "rgb(40, 44, 52) !important;"  # Dark gray background (change as needed)
        #     }
        # },
        {
            'selector': 'node',
...
def get_dags(name):
    ...
    if name == "Original":
        if st.session_state.ANALYSIS_RESULT:
            return ([get_original_simple_dag(st.session_state.ANALYSIS_RESULT.original_dag, reuse_info)],
                    [st.session_state.ANALYSIS_RESULT.original_dag])
    elif name == "Variants":
        if st.session_state.ANALYSIS_RESULT:
...
def render_full_size_dag(stage_name):
    ...
    if st.session_state.ANALYSIS_RESULT:
        ...
        selected = render_dag_slot(stage_name, visualization_dag, f"full-size-{stage_name}", height='750px',
                                   description=False)
...
def render_shadow_dags():
    st.markdown("These are the automatically generated Shadow Pipeline DAGs. You can click on nodes to learn "
                "more about them. For example, to see the corresponding code from the original pipeline, the "
                "runtime of an operator, and the shape of its output.")
    if st.session_state.ANALYSIS_RESULT:
        for shadow_pipeline, internal_dag in st.session_state.ANALYSIS_RESULT.shadow_pipeline_to_dags.items():
            header = get_readable_shadow_header(shadow_pipeline)
            st.subheader(header)
            reuse_info = st.session_state.ANALYSIS_RESULT.dag_extraction_info.reuse_info
            visualisation_dag = get_original_simple_dag(internal_dag, reuse_info)
            selected = render_dag_slot(header, visualisation_dag, f"full-size-{header}", height='200px',
                                       description=False)
            if len(selected['nodes']) != 0:
                render_dag_node_details(internal_dag, selected, width=1000, table_instead_of_df=True)
            else:
...
def render_dag_node_details(internal_dag, selected, width=300, table_instead_of_df=False):
    with st.container():
        ...
        attribute_names = ['Node ID', 'Operator Type', "Source Code", "Description", "Runtime", "Shape",
        ...
        if selected_node.optional_code_info is not None:
            ...
            end_lineno = None
            end_col_offset = None
        if selected_node.details.optimizer_info is not None and selected_node.details.optimizer_info.runtime is not None:
            runtime = f"{selected_node.details.optimizer_info.runtime:.2f} ms"
            shape = f"{selected_node.details.optimizer_info.shape}"
        else:
            runtime = "<NA>"
            shape = "<NA>"
        attribute_values = [node_id,
                            selected_node.operator_info.operator.value,
                            source_code,
...
def get_readable_shadow_header(shadow):
    if type(shadow) == LabelErrors:
        shadow_header = "Label Errors"
    elif type(shadow) == DataErrorRobustness:
        shadow_header = "Data Error Robustness"
    elif type(shadow) == FairnessSlices:
        shadow_header = "Fairness Slices"
    else:
        raise NotImplementedError("TODO")
        # report.loc[report["error"] == "mislabel", "corrupted_column"] = "LABELS"
    return shadow_header

def sanitize_dataframe(df: pandas.DataFrame) -> pandas.DataFrame:
    if isinstance(df, pandas.DataFrame):
        sanitized_df = df.copy()
        for col in sanitized_df.columns:
            # If the column has lists, flatten the lists (if they have only one element, keep it as a single value)

            if sanitized_df[col].apply(lambda x: isinstance(x, (list, numpy.ndarray))).any():
                sanitized_df[col] = sanitized_df[col].apply(lambda x: x[0] if isinstance(x, (list, numpy.ndarray)) and len(x) == 1 else x)
            # Convert any object columns to strings (except for lists)
            elif sanitized_df[col].dtype == 'object':
                # sanitized_df[col] = sanitized_df[col].astype(str).apply(lambda x: x[:25] + "..." if len(x) > 30 else x)  # Truncate to 30 characters
                sanitized_df[col] = sanitized_df[col].astype(str)
        # sanitized_df =  sanitized_df.head(4)
        return sanitized_df
    else:
        return df
